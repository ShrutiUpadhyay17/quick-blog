...
const CommentTableItem = ({ comment, fetchComments }) => {
    ...
    const deleteComment = async () => {
        try {
            const confrim = window.confirm("Are you sure to delete this comment?");
            if(!confrim) return;
        ...
        <tr className="order-y border-gray-300">
            ...
            <td className="px-6 py-4">
                <div className="inline-flex items-center gap-4">
                    {
                    !comment.isApproved ?
                    <img onClick={approveComment} src={assets.tick_icon} className="w-5 hover:scale-110
                    transition-all cursor-pointer"/> : 
                    <p className="text-xs border border-green-600 bg-green-100 
                    text-green-600 rounded-full px-3 py-1 ">
                        Approved</p> 
                    }
